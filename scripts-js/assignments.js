...
const findData = () => {
    const recordsTable = document.getElementById("grid_translation_jobs_grid_records");
    const columnsTable = document.getElementById("grid_translation_jobs_grid_columns");
    ...
    const headerRow = columnsTable.querySelector("tr");
    if (!headerRow) {
    ...
    for (const row of rows) {
        ...
        if (!lineAttr || lineAttr === "0" || lineAttr === "top")
            continue; // skip placeholder rows
        // Grab cells using the dynamic column numbers
        const titleCell = row.querySelector(`td[col="${colIndexes["Title"]}"] div`);
        const clientsCell = row.querySelector(`td[col="${colIndexes["Clients"]}"] div`);
        const runtimeCell = row.querySelector(`td[col="${colIndexes["Runtime"]}"] div`);
        const versionLink = row.querySelector(`td[col="${colIndexes["Assignment Due Date"]}"] div`);
        // Stop crawling if any required data is missing
        if (!titleCell || !clientsCell || !runtimeCell || !versionLink)
            break;
