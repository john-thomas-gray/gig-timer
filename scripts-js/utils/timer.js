...
const getStoredDuration = () => new Promise((resolve) => {
    ...
    storageLocal.get(["lastSessionDurationMs", "projectTitle", TIMER_PROJECT_DURATIONS_KEY], (result) => {
        ...
        const durationMap = normalizeTimerDurationMap(result[TIMER_PROJECT_DURATIONS_KEY]);
        if (currentProject &&
...
const storeDuration = (duration) => {
    ...
    storageLocal.get(["projectTitle", TIMER_PROJECT_DURATIONS_KEY], (result) => {
        if (chrome?.runtime?.lastError) {
            console.warn("Failed to read session duration map.", chrome.runtime.lastError);
            return;
        }
...
async function startTimerInternal() {
    ...
    logIntervalId = window.setInterval(() => {
        ...
        const totalDurationMs = accumulatedDurationBeforeSessionMs + sessionDurationMs;
